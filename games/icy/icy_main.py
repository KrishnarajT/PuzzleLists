...
START_BTN_UPPER = (WIDTH - floor(WIDTH / 13.2), HEIGHT - floor(HEIGHT / 3.2))

EXIT_BTN_LOWER = (WIDTH - floor(WIDTH / 4.5), HEIGHT - floor(HEIGHT / 4.32))
EXIT_BTN_UPPER = (WIDTH - floor(WIDTH / 13.2), HEIGHT - floor(HEIGHT / 10.8))

TIME_LIM_BTN_LOWER = (WIDTH - floor(WIDTH / 1.46), HEIGHT - floor(HEIGHT / 1.6))
TIME_LIM_BTN_UPPER = (WIDTH - floor(WIDTH / 3.14), HEIGHT - floor(HEIGHT / 2.51))
...
def ShowRevealer(box_Side):
    ...
    pygame.draw.rect(
        WIN,
        Reveal_Surface_Color,
        (WEIRD_X - 2, WEIRD_Y - 2, box_Side + 2, box_Side + 2),
        2,
    )
...
def drawAndAssignBG_BOX(BG_Color, BG_Size=10):
    ...
    BG_BOX_X, BG_BOX_Y = floor(WIDTH / BG_Size), floor(
        HEIGHT / BG_Size
...
def drawStatsClick(cur_level, highscore):
    ...
    SUBSURFACE = PLAY_IMAGE.subsurface(
        (
            floor(WIDTH - WIDTH / 7.7),
            floor(HEIGHT / 1.4),
            tap_label.get_width(),
            tap_label.get_height(),
        )
    )
...
def clickLimit():
    ...
    while loop_Click:
        ...
        for event in pygame.event.get():
            ...
            if checkMouse(
                BG_BOX_X + DRAW_BUFFER,
                BG_BOX_Y + DRAW_BUFFER,
                (BG_BOX_X + BG_BOX_SIDE - DRAW_BUFFER),
                (BG_BOX_Y + BG_BOX_SIDE - DRAW_BUFFER),
                event,
            ):
                TAP += 1
                if event.type == pygame.MOUSEBUTTONUP:
...
def mainMenu():
    ...
    while loop_Menu != 0:
        ...
        for event in pygame.event.get():
            ...
            if checkMouse(
                TIME_LIM_BTN_LOWER[0],
                TIME_LIM_BTN_LOWER[1],
                TIME_LIM_BTN_UPPER[0],
                TIME_LIM_BTN_UPPER[1],
                event,
            ):
                loop_Menu = timeLimit()

            if checkMouse(
                CLICK_LIM_BTN_LOWER[0],
                CLICK_LIM_BTN_LOWER[1],
                CLICK_LIM_BTN_UPPER[0],
                CLICK_LIM_BTN_UPPER[1],
                event,
            ):
                loop_Menu = clickLimit()
