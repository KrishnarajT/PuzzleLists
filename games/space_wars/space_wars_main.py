...
class Player(GenShip):
    ...
    def __init__(self, x, y, Health=100):
        ...
        self.Max_Health = (
            Health  # coz the max health is going to be the health that we give it,
        )
    ...
    def PlayerHealthBar_(self, Window):
        pygame.draw.rect(
            ...
            (
                round(self.x),
                round(self.y + self.Ship_Image.get_height() + 10),
                round(self.Ship_Image.get_width()),
                10,
            ),
        )
        pygame.draw.rect(
            ...
            (
                round(self.x),
                round(self.y + self.Ship_Image.get_height() + 10),
                round(self.Ship_Image.get_width() * (self.Health / self.Max_Health)),
                10,
            ),
        )
...
def checkInput(player_Ship_Obj, player_Ship_Vel):
    ...
    if keys[pygame.K_w] and player_Ship_Obj.y >= 0:
        player_Ship_Obj.y -= (
            player_Ship_Vel  # this changes the value of the self variable of the object
        )
    if keys[pygame.K_a] and player_Ship_Obj.x >= 0:
        player_Ship_Obj.x -= player_Ship_Vel
    if (
        keys[pygame.K_s]
        and player_Ship_Obj.y + player_Ship_Obj.GetHeight_() + 20 <= HEIGHT
    ):
...
def drawText(level, life, player_Obj, won_Level=False):
    ...
    score_Label = main_Font.render(
        f"SCORE :  {player_Obj.CurHighScore}", 1, color.get("Green")
    )
    high_Score_Label = main_Font.render(
        f"HIGH SCORE :  {player_Obj.HighScore}", 1, color.get("Green")
    )
    ...
    WIN.blit(
        score_Label,
        (WIDTH - score_Label.get_width() - 10, level_Label.get_height() + 10),
    )
    WIN.blit(
        high_Score_Label,
        (
            WIDTH - high_Score_Label.get_width() - 10,
            (level_Label.get_height() * 2) + 10,
        ),
    )

    # CHANGE LEVEL
    if won_Level:
        level_Won_Label = level_Won_Font.render(
            f"L :  {level} ", True, color.get("White")
        )
        WIN.blit(
            level_Won_Label,
            (
                int((WIDTH / 2 - level_Won_Label.get_width() / 2)),
...
def main():
    ...
    player_Obj.x = (
        WIDTH / 2 - player_Obj.GetWidth_() / 2
    )  # assigning the val of x to player obj
    player_Obj.y = (
        HEIGHT * 0.75 - player_Obj.GetHeight_() / 2
    )  # assigning the val of y to obj, so its in the center.
    ...
    while loop:
        ...
        if len(enemies) == 0:  # this happens when you defeat all the enemies
            ...
            for i in range(0, enemies_InWave):
                ...
                Enemy_Obj = Enemy(
                    enemy_Ship_RandX, enemy_Ship_RandY, enemy_Ship_Rand_Color
                )
            ...
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_ESCAPE:
                    pygame.display.set_mode(
                        (WIDTH, HEIGHT)
                    )  # this makes the x button appear
